...
class OpImportAnm(bpy.types.Operator, io_utils.ImportHelper):
    ...
    def execute(self, context):
        if len(self.files) == 0:
            self.report({'ERROR'}, 'No files selected')
            return {'CANCELLED'}
        from .fmt_anm import import_file, Context
        cx = Context(as_camera_animation=self.camera_animation)
        for file in self.files:
            import os.path
            ext = os.path.splitext(file.name)[-1].lower()
            if ext == '.anm':
...
class OpExportAnm(bpy.types.Operator, io_utils.ExportHelper):
    ...
    def execute(self, context):
        if not self.filepath:
            self.report({'ERROR'}, 'No file selected')
            return {'CANCELLED'}
        from .fmt_anm import export_file, Context
        cx = Context(as_camera_animation=False)
